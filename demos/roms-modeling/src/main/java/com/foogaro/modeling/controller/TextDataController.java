...
public class TextDataController {
    ...
    @GetMapping(path="/findByMeasurementsIn/{measurement}")
    public ResponseEntity<List<TextData>> findByMeasurementsIn(@PathVariable double measurement) {
        return Optional.ofNullable(service.findByMeasurementsIn(measurement))
                .filter(list -> !list.isEmpty())
                .map(ResponseEntity::ok)
                .orElse(ResponseEntity.notFound().build());
    }

    @GetMapping(path="/findByMissingDescription")
    public ResponseEntity<List<TextData>> findByMissingDescription() {
        return Optional.ofNullable(service.findByMissingDescription())
                .filter(list -> !list.isEmpty())
                .map(ResponseEntity::ok)
                .orElse(ResponseEntity.notFound().build());
    }
