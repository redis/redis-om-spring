...
public class TextDataService {
    ...
    public List<TextData> findByDescription(String description) { return repository.findByDescription(description); }
    public List<TextData> findByYear(int year) { return repository.findByYear(year); }
    public List<TextData> findByYearBetween(int fromYear, int toYear) { return repository.findByYearBetween(fromYear, toYear); }
    public List<TextData> findByScore(double score) { return repository.findByScore(score); }
    public List<TextData> findByScoreBetween(double fromScore, double toScore) { return repository.findByScoreBetween(fromScore, toScore); }
    ...
    private List<TextData> semanticSearch(String prompt, int limit)  {
        ...
        List<Pair<TextData,Double>> matchWithScore = stream
                .filter(TextData$.TEXT_EMBEDDING.knn(limit, embeddedQuery))
                .sorted(TextData$._TEXT_EMBEDDING_SCORE, SortedField.SortOrder.ASC)
                .limit(limit)
                .map(Fields.of(TextData$._THIS, TextData$._TEXT_EMBEDDING_SCORE))
    ...
    public List<TextData> hybridSearch(TextData textData)  {
        ...
        SearchStream<TextData> stream = entityStream.of(TextData.class);
        List<Pair<TextData,Double>> matchWithScore = stream
                .filter(TextData$.TEXT_EMBEDDING.knn(numberOfNearestNeighbors, embeddedQuery))
                .filter(TextData$.YEAR.between(textData.getYear(), textData.getYear()))
                .sorted(TextData$._TEXT_EMBEDDING_SCORE, SortedField.SortOrder.ASC)
