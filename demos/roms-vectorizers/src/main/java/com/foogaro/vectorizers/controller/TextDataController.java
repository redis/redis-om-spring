...
public class TextDataController {
  ...
  public ResponseEntity<List<TextData>> semanticSearch(@RequestBody String prompt) {
    return Optional.ofNullable(service.semanticSearch(prompt)).filter(list -> !list.isEmpty()).map(ResponseEntity::ok)
        .orElse(ResponseEntity.notFound().build());
  }
  ...
  public ResponseEntity<List<TextData>> hybridSearch(@RequestBody TextData textData) {
    return Optional.ofNullable(service.hybridSearch(textData)).filter(list -> !list.isEmpty()).map(ResponseEntity::ok)
        .orElse(ResponseEntity.notFound().build());
  }
