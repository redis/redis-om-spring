...
class VectorizeOpenAIHashTest extends AbstractBaseEnhancedRedisTest {
  ...
  void loadTestData() throws IOException {
    if (repository.count() == 0) {
      repository.save(HashWithOpenAIEmbedding.of("cat",
      ...
      repository.save(HashWithOpenAIEmbedding.of("face2",
          "The person box was packed with jelly many dozens of months later."));
    }

    if (repository2.count() == 0) {
      repository2.save(HashWithCustomModelOpenAIEmbedding.of("cat",
          "The cat (Felis catus) is a domestic species of small carnivorous mammal."));
      repository2.save(HashWithCustomModelOpenAIEmbedding.of("cat2",
          "It is the only domesticated species in the family Felidae and is commonly referred to as the domestic cat or house cat"));
      repository2.save(HashWithCustomModelOpenAIEmbedding.of("catdog",
          "This is a picture of a cat and a dog together"));
      repository2.save(HashWithCustomModelOpenAIEmbedding.of("face",
          "Three years later, the coffin was still full of Jello."));
      repository2.save(HashWithCustomModelOpenAIEmbedding.of("face2",
          "The person box was packed with jelly many dozens of months later."));
    }
  }
  ...
  void testKnnSentenceSimilaritySearchWithScores() {
    ...
    assertAll( //
        () -> assertThat(results).hasSize(5).map(Pair::getFirst).map(HashWithOpenAIEmbedding::getName).containsExactly(
            "cat", "cat2", "catdog", "face2", "face"), //
