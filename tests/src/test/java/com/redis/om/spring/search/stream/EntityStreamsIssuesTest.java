...
class EntityStreamsIssuesTest extends AbstractBaseDocumentTest {
  ...
  void beforeEach() throws IOException {
    ...
    if (deepNestNonIndexedRepository.count() == 0) {
      DeepNestNonIndexed dnni1 = DeepNestNonIndexed.of("dn-1", NestLevelNonIndexed1.of("nl-1-1",
          "Louis, I think this is the beginning of a beautiful friendship.", NestLevelNonIndexed2.of("nl-2-1",
              "Here's looking at you, kid.")));
      DeepNestNonIndexed dnni2 = DeepNestNonIndexed.of("dn-2", NestLevelNonIndexed1.of("nl-1-2",
          "Whoever you are, I have always depended on the kindness of strangers.", NestLevelNonIndexed2.of("nl-2-2",
              "Hey, you hens! Cut out the cackling in there!")));
      DeepNestNonIndexed dnni3 = DeepNestNonIndexed.of("dn-3", NestLevelNonIndexed1.of("nl-1-3",
          "A good body with a dull brain is as cheap as life itself.", NestLevelNonIndexed2.of("nl-2-3",
              "I'm Spartacus!")));
    ...
    if (doc2Repository.count() == 0) {
      ...
      IntStream.range(0, 31).forEach(i -> doc2s.add(Doc2.of(String.format("Marca%s Modelo %s", i, i), String.format(
          "COLOR %s", i))));
      doc2Repository.saveAll(doc2s);
    }

    if (deepListRepository.count() == 0) {
      List<NestLevel2> list1 = List.of(NestLevel2.of("nl-1-1", "It's just a flesh wound!"), NestLevel2.of("nl-1-2",
          "Nobody expects the Spanish Inquisition!"), NestLevel2.of("nl-1-3", "I fart in your general direction!"));
      List<NestLevel2> list2 = List.of(NestLevel2.of("nl-2-1", "We are the knights who say 'Ni!"), NestLevel2.of(
          "nl-2-2", "And now for something completely different."), NestLevel2.of("nl-2-3",
              "What have the Romans ever done for us?"));
      List<NestLevel2> list3 = List.of(NestLevel2.of("nl-3-1", "I have a very silly job and I take it very seriously."),
          NestLevel2.of("nl-3-2", "What's brown and sticky? A stick!"), NestLevel2.of("nl-3-3",
  ...
  void testFreeFormTextSearchOrderIssue() {
    ...
    var withFreeTextFirst = entityStream.of(Doc.class).filter("*co*").filter(Doc$.FIRST.eq("Microsoft")).collect(
        Collectors.toList());

    var withFreeTextLast = entityStream.of(Doc.class).filter(Doc$.FIRST.eq("Microsoft")).filter("*co*").collect(
        Collectors.toList());
  ...
  void testLongGePredicate() {
    ...
    assertAll( //
        () -> assertThat(results).hasSize(4), () -> assertThat(results).extracting("id").containsExactly("doc-3",
            "doc-4", "doc-5", "doc-6"));
  }
  ...
  void testLongGtPredicate() {
    ...
    assertAll( //
        () -> assertThat(results).hasSize(3), () -> assertThat(results).extracting("id").containsExactly("doc-4",
            "doc-5", "doc-6"));
  }
  ...
  void testLongInPredicate() {
    ...
    assertAll( //
        () -> assertThat(results).hasSize(3), () -> assertThat(results).extracting("id").containsExactly("doc-2",
            "doc-4", "doc-6"));
  }
  ...
  void testLongLePredicate() {
    ...
    assertAll( //
        () -> assertThat(results).hasSize(3), () -> assertThat(results).extracting("id").containsExactly("doc-1",
            "doc-2", "doc-3"));
  }
  ...
  void testLongLtPredicate() {
    ...
    assertAll( //
        () -> assertThat(results).hasSize(2), () -> assertThat(results).extracting("id").containsExactly("doc-1",
            "doc-2"));
  }
  ...
  void testLongBetweenPredicate() {
    ...
    assertAll( //
        () -> assertThat(results).hasSize(4), () -> assertThat(results).extracting("id").containsExactly("doc-2",
            "doc-3", "doc-4", "doc-5"));
  }
  ...
  void testLongNotEqPredicate() {
    ...
    assertAll( //
        () -> assertThat(results).hasSize(5), () -> assertThat(results).extracting("id").containsExactly("doc-1",
            "doc-2", "doc-3", "doc-4", "doc-6"));
  }
  ...
  void testMapEntityStreamsReturnNullValue() {
    ...
    var result = entityStream.of(Person.class).filter(Person$.NAME.eq("PersonWithoutEmail")).map(Fields.of(Person$.NAME,
        Person$.EMAIL)) // should handle empty email as mapped return value
  ...
  void testMapEntityStreamsReturnNestedField() {
    var results = entityStream.of(DeepNest.class).filter(DeepNest$.NEST_LEVEL1_NEST_LEVEL2_NAME.eq("nl-2-2")).map(
        DeepNest$.NEST_LEVEL1_NEST_LEVEL2_NAME) // should handle nested property as mapped return value
        .collect(Collectors.toList());
  ...
  void testMapEntityStreamsReturnIndexedBooleanValue() {
    ...
    var result = entityStream.of(DocWithBoolean.class).filter(DocWithBoolean$.ID.eq(docWithBoolean.getId())).map(
        DocWithBoolean$.INDEXED_BOOLEAN) // should handle returned indexed boolean value, but fails
        .collect(Collectors.toList());
  ...
  void testMapEntityStreamsReturnNonIndexedBooleanValue() {
    ...
    var result = entityStream.of(DocWithBoolean.class).filter(DocWithBoolean$.ID.eq(docWithBoolean.getId())).map(
        DocWithBoolean$.NON_INDEXED_BOOLEAN) // should handle returned non indexed boolean value, succeeds
        .collect(Collectors.toList());
    assertThat(result).containsOnly(true);
  }
  ...
  void testNonIndexedReturnedFields() {
    ...
    var result = entityStream.of(KitchenSink.class).filter(KitchenSink$.ID.eq("ks0")).map(allFields).collect(Collectors
        .toList());
    ...
    assertAll( //
        () -> assertThat(ksValues.getFirst()).isEqualTo(localDate), () -> assertThat(ksValues.getSecond())
            .isEqualToIgnoringNanos(localDateTime), () -> assertThat(ksValues.getThird()).isEqualTo(date),
        () -> assertThat(ksValues.getFourth()).isEqualTo(point), () -> assertThat(ksValues.getFifth()).isEqualTo(ulid),
        () -> assertThat(ksValues.getSixth()).containsExactlyInAnyOrderElementsOf(setThings), () -> assertThat(ksValues
            .getSeventh()).containsExactlyInAnyOrderElementsOf(listThings));
  }
  ...
  void testNonIndexedReturnedFieldsWithEmptyCollections() {
    var allFields = Fields.of( //
        KitchenSink$.LOCAL_DATE, KitchenSink$.LOCAL_DATE_TIME, KitchenSink$.DATE, KitchenSink$.POINT, KitchenSink$.ULID,
        KitchenSink$.SET_THINGS, KitchenSink$.LIST_THINGS);
    var result = entityStream.of(KitchenSink.class).filter(KitchenSink$.ID.eq("ks1")).map(allFields).collect(Collectors
        .toList());
    ...
    assertAll( //
        () -> assertThat(ksValues.getFirst()).isEqualTo(localDate), () -> assertThat(ksValues.getSecond())
            .isEqualToIgnoringNanos(localDateTime), () -> assertThat(ksValues.getThird()).isEqualTo(date),
        () -> assertThat(ksValues.getFourth()).isEqualTo(point), () -> assertThat(ksValues.getFifth()).isEqualTo(ulid),
        () -> assertThat(ksValues.getSixth()).isNull(), () -> assertThat(ksValues.getSeventh()).isNull());
  }
  ...
  void testNonIndexedReturnedFieldsWithNullCollections() {
    ...
    var result = entityStream.of(KitchenSink.class).filter(KitchenSink$.ID.eq("ks2")).map(allFields).collect(Collectors
        .toList());
  ...
  void testReturnedFieldsDeepNested() {
    ...
    var result = entityStream.of(DeepNest.class).filter(DeepNest$.NAME.eq("dn-1")).map(allFields).collect(Collectors
        .toList());
    ...
    assertAll( //
        ...
        () -> assertThat(values.getThird()).isEqualTo(
            "Louis, I think this is the beginning of a beautiful friendship."), () -> assertThat(values.getFourth())
                .isEqualTo("nl-2-1"), () -> assertThat(values.getFifth()).isEqualTo("Here's looking at you, kid."));
  }
