...
/**
 * Implementation of a tuple mapper for 16-element tuples (Sexdecuple).
 * This class provides functionality to map from a source type T to a Sexdecuple containing sixteen mapped values.
 * It is particularly useful in Redis OM Spring for complex data transformations, query result mapping, and projection
 * scenarios where sixteen related properties from an object need to be extracted and grouped as a tuple.
 *
 * @param <T>   the input type to map from
 * @param <T0>  the type of the first element in the resulting sexdecuple
 * @param <T1>  the type of the second element in the resulting sexdecuple
 * @param <T2>  the type of the third element in the resulting sexdecuple
 * @param <T3>  the type of the fourth element in the resulting sexdecuple
 * @param <T4>  the type of the fifth element in the resulting sexdecuple
 * @param <T5>  the type of the sixth element in the resulting sexdecuple
 * @param <T6>  the type of the seventh element in the resulting sexdecuple
 * @param <T7>  the type of the eighth element in the resulting sexdecuple
 * @param <T8>  the type of the ninth element in the resulting sexdecuple
 * @param <T9>  the type of the tenth element in the resulting sexdecuple
 * @param <T10> the type of the eleventh element in the resulting sexdecuple
 * @param <T11> the type of the twelfth element in the resulting sexdecuple
 * @param <T12> the type of the thirteenth element in the resulting sexdecuple
 * @param <T13> the type of the fourteenth element in the resulting sexdecuple
 * @param <T14> the type of the fifteenth element in the resulting sexdecuple
 * @param <T15> the type of the sixteenth element in the resulting sexdecuple
 */
public final class SexdecupleMapperImpl<T, T0, T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T11, T12, T13, T14, T15> extends
    AbstractTupleMapper<T, Sexdecuple<T0, T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T11, T12, T13, T14, T15>> {

  /**
   * Constructs a new SexdecupleMapperImpl with the specified mapping functions.
   *
  ...
  /**
   * Applies all sixteen mapping functions to the input and creates a sexdecuple with the results.
   *
   * @param t the input to be mapped
   * @return a sexdecuple containing the mapped values
   */
  @Override
    ...
    return Tuples.of(getFirst().apply(t), getSecond().apply(t), getThird().apply(t), getFourth().apply(t), getFifth()
        .apply(t), getSixth().apply(t), getSeventh().apply(t), getEighth().apply(t), getNinth().apply(t), getTenth()
            .apply(t), getEleventh().apply(t), getTwelfth().apply(t), getThirteenth().apply(t), getFourteenth().apply(
                t), getFifteenth().apply(t), getSixteenth().apply(t));
  }
