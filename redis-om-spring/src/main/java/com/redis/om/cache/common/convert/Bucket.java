 ...
 * {@link RedisData}. It provides a container for storing and manipulating
 * key-value pairs that will be persisted as Redis hash structures.
 *
 */
@SuppressWarnings(
  "deprecation"
...
public class Bucket {
   ...
   *
   * @param path must not be {@literal null} or {@link String#isEmpty()}.
   * @return {@literal true} if the {@code path} is associated with a
   *         non-{@code null} value.
   * @since 2.5
   */
   ...
   * @return a new bucket containing only the key/value pairs with the specified prefix
   */
  ...
  /**
   * Get the {@link BucketPropertyPath} for a given property within the current
   * {@link Bucket}.
   *
  ...
  public static Bucket newBucketFromRawMap(Map<byte[], byte[]> source) {
    ...
    for (Entry<byte[], byte[]> entry : source.entrySet()) {
  ...
  public static Bucket newBucketFromStringMap(Map<String, String> source) {
    ...
    for (Entry<String, String> entry : source.entrySet()) {
      bucket.put(entry.getKey(), StringUtils.hasText(entry.getValue()) ?
  ...
  /**
   * Value object representing a path within a {@link Bucket}. Paths can be either
   * top-level (if the {@code prefix} is {@literal null} or empty) or nested with
   * a given {@code prefix}.
   *
   */
  public static class BucketPropertyPath {
    ...
    /**
     * Creates a {@link BucketPropertyPath} given {@link Bucket} and {@code prefix}.
     * The resulting path is top-level if {@code prefix} is empty or nested, if
     * {@code prefix} is not empty.
     *
     * @param bucket the bucket, must not be {@literal null}.
     * @param prefix the prefix. Property path is top-level if {@code prefix} is
     *               {@literal null} or empty.
     * @return {@link BucketPropertyPath} within the given {@link Bucket} using
     *         {@code prefix}.
     */
    ...
    /**
     * Gets the bucket associated with this property path.
     *
    ...
    /**
     * Gets the prefix for this property path.
     *
     * @return the prefix string, may be {@literal null} for top-level paths
     */
    @Nullable
