...
import com.redis.om.cache.common.convert.PartialUpdate.PropertyUpdate;
import com.redis.om.cache.common.convert.PartialUpdate.UpdateCommand;

/**
 * {@link RedisConverter} implementation creating flat binary map structure out
 * of a given domain type. Considers {@link Indexed} annotation for enabling
 * helper structures for finder operations. <br />
 * <br />
 * <strong>NOTE</strong> {@link MappingRedisConverter} is an
 * {@link InitializingBean} and requires
...
public class MappingRedisConverter implements RedisConverter, InitializingBean {
  ...
  /**
   * Creates new {@link MappingRedisConverter} and defaults
   ...
   */
  public MappingRedisConverter(@Nullable RedisMappingContext mappingContext,
  ...
  private <R> R doReadInternal(String path, Class<R> type, RedisData source) {
    ...
    Object instance = instantiator.createInstance((RedisPersistentEntity<RedisPersistentProperty>) entity,
        new PersistentEntityParameterValueProvider<>(entity, new ConverterAwareParameterValueProvider(path, source,
  ...
  /**
   * Reads a property value from the Redis data source.
   *
   * @param path               the path to the property
   * @param source             the Redis data source
   * @param persistentProperty the property to read
   * @return the property value, or {@literal null} if not found
   */
  @Nullable
  protected Object readProperty(String path, RedisData source, RedisPersistentProperty persistentProperty) {
    ...
    if (typeInformation.isMap()) {
      ...
      return readMapOfComplexTypes(currentPath, typeInformation.getType(), typeInformation.getRequiredComponentType()
          .getType(), mapValueType, source);
    }
    ...
    if (mappingContext.getPersistentEntity(typeInformation) != null && !conversionService.canConvert(byte[].class,
        typeInformation.getRequiredActualType().getType())) {
  ...
  /**
   * Writes a partial update to the Redis data sink.
   * 
   * @param update the partial update to write
   * @param sink   the Redis data sink to write to
   */
  ...
  private void writePartialPropertyUpdate(PartialUpdate<?> update, PropertyUpdate pUpdate, RedisData sink,
    ...
    if (targetProperty.isAssociation()) {
      ...
      writeCollection(entity.getKeySpace(), pUpdate.getPropertyPath(), collection, targetProperty.getTypeInformation()
  ...
  private void writeInternal(@Nullable String keyspace, String path, @Nullable Object value,
    ...
    entity.doWithProperties((PropertyHandler<RedisPersistentProperty>) persistentProperty -> {
      ...
      if (persistentProperty.isMap()) {
        ...
        if (propertyValue == null) {
          writeCollection(keyspace, propertyStringPath, null, persistentProperty.getTypeInformation()
              .getRequiredComponentType(), sink);
        } else {

          if (Iterable.class.isAssignableFrom(propertyValue.getClass())) {

            writeCollection(keyspace, propertyStringPath, (Iterable<?>) propertyValue, persistentProperty
                .getTypeInformation().getRequiredComponentType(), sink);
          } else if (propertyValue.getClass().isArray()) {

            writeCollection(keyspace, propertyStringPath, CollectionUtils.arrayToList(propertyValue), persistentProperty
                .getTypeInformation().getRequiredComponentType(), sink);
          } else {
  ...
  private void writeToBucket(String path, @Nullable Object value, RedisData sink, Class<?> propertyType) {
    ...
    if (customConversions.hasCustomWriteTarget(value.getClass())) {
      ...
      if (!propertyType.isPrimitive() && !targetType.filter(it -> ClassUtils.isAssignable(Map.class, it))
          .isPresent() && customConversions.isSimpleType(value.getClass()) && value.getClass() != propertyType) {
        typeMapper.writeType(value.getClass(), sink.getBucket().getPropertyPath(path));
      }
   ...
   *
   * @param source the object to convert to binary representation
   * @return the binary representation as byte array
   * @throws ConverterNotFoundException if no suitable converter can be found
   */
  ...
  /**
   * Sets the reference resolver to be used for resolving references.
   * 
   * @param referenceResolver the reference resolver to use
   */
   ...
   *
   * @param customConversions the custom conversions to be used for type conversion
   */
  ...
  public static class KeyspaceIdentifier {
    ...
    /**
     * Constant representing a phantom key identifier.
     */
    public static final String PHANTOM = "phantom";

    /**
     * Delimiter used to separate keyspace and ID in a key.
     */
    public static final String DELIMITER = ":";

    /**
     * Suffix appended to phantom keys, consisting of the delimiter followed by the phantom identifier.
     */
    ...
    /**
     * Returns the keyspace part of the identifier.
     * 
    ...
    /**
     * Returns the ID part of the identifier.
     * 
    ...
    /**
     * Indicates whether this is a phantom key.
     * 
  ...
  public static class BinaryKeyspaceIdentifier {
    ...
    /**
     * Binary representation of the phantom key identifier.
     */
    public static final byte[] PHANTOM = KeyspaceIdentifier.PHANTOM.getBytes();

    /**
     * Delimiter byte used to separate keyspace and ID in a binary key.
     */
    public static final byte DELIMITER = ':';

    /**
     * Binary suffix appended to phantom keys, consisting of the delimiter followed by the phantom identifier.
     */
    ...
    /**
     * Returns the binary keyspace part of the identifier.
     * 
     * @return the keyspace as a byte array
     */
    ...
    /**
     * Returns the binary ID part of the identifier.
     * 
