...
public class MetamodelGeneratorTest {
  ...
  void testValidDocumentIndexed(Results results) throws IOException {
    ...
    assertThat(fileContents).contains("createdDate = ValidDocumentIndexed.class.getDeclaredField(\"createdDate\");");
    ...
    assertThat(fileContents).contains("yearFounded = ValidDocumentIndexed.class.getDeclaredField(\"yearFounded\");");
    assertThat(fileContents).contains("name = ValidDocumentIndexed.class.getDeclaredField(\"name\");");
    assertThat(fileContents).contains("location = ValidDocumentIndexed.class.getDeclaredField(\"location\");");
    assertThat(fileContents).contains("tags = ValidDocumentIndexed.class.getDeclaredField(\"tags\");");

    // test Metamodel Field generation
    assertThat(fileContents).contains("public static NonIndexedNumericField<ValidDocumentIndexed, Date> CREATED_DATE;");
    assertThat(fileContents)
    ...
    assertThat(fileContents).contains("public static BooleanField<ValidDocumentIndexed, Boolean> PUBLICLY_LISTED;");
    assertThat(fileContents).contains("public static DateField<ValidDocumentIndexed, LocalDate> LAST_VALUATION;");
    assertThat(fileContents).contains("public static NonIndexedTextField<ValidDocumentIndexed, String> ID;");
    assertThat(fileContents).contains("public static NumericField<ValidDocumentIndexed, Integer> YEAR_FOUNDED;");
    ...
    assertThat(fileContents).contains("ID = new NonIndexedTextField<ValidDocumentIndexed, String>(id,false);");
    assertThat(fileContents)
  ...
  void testValidDocumentUnindexed(Results results) throws IOException {
    ...
    assertThat(fileContents).contains("setThings = ValidDocumentUnindexed.class.getDeclaredField(\"setThings\");");
    assertThat(fileContents)
    ...
    assertThat(fileContents).contains("listThings = ValidDocumentUnindexed.class.getDeclaredField(\"listThings\");");
    assertThat(fileContents).contains("date = ValidDocumentUnindexed.class.getDeclaredField(\"date\");");
    assertThat(fileContents).contains("localDate = ValidDocumentUnindexed.class.getDeclaredField(\"localDate\");");
    ...
    // test Metamodel Field generation
    assertThat(fileContents).contains("public static NonIndexedTextField<ValidDocumentUnindexed, String> ID;");
    assertThat(fileContents)
    ...
    assertThat(fileContents).contains("public static NonIndexedTextField<ValidDocumentUnindexed, String> STRING_;");
    assertThat(fileContents).contains("public static NonIndexedBooleanField<ValidDocumentUnindexed, Boolean> BOOL;");

    // test Metamodel Field initialization
    assertThat(fileContents).contains("ID = new NonIndexedTextField<ValidDocumentUnindexed, String>(id,false);");
    assertThat(fileContents)
    ...
    assertThat(fileContents).contains("POINT = new MetamodelField<ValidDocumentUnindexed, Point>(point,false);");
    assertThat(fileContents)
        .contains("LIST_THINGS = new NonIndexedTagField<ValidDocumentUnindexed, List<String>>(listThings,false);");
    assertThat(fileContents).contains("DATE = new NonIndexedNumericField<ValidDocumentUnindexed, Date>(date,false);");
    assertThat(fileContents)
...
//        "Processing class ValidDocumentIndexedNested could not resolve valid.Address checking for nested indexables");
    ...
    assertThat(fileContents).contains("address = ValidDocumentIndexedNested.class.getDeclaredField(\"address\");");

    // test Metamodel Field generation
    assertThat(fileContents).contains("public static NonIndexedTextField<ValidDocumentIndexedNested, String> ID;");
    assertThat(fileContents).contains("public static MetamodelField<ValidDocumentIndexedNested, Address> ADDRESS;");

    // test Metamodel Field initialization
    assertThat(fileContents).contains("ID = new NonIndexedTextField<ValidDocumentIndexedNested, String>(id,false);");
    assertThat(fileContents)
...
//    assertThat(fileContents).contains("ADDRESS_CITY = new String(\"address_city\");");
  ...
  public void testValidDocumentUnindexedWithoutPackage(Results results) throws IOException {
    assertThat(results.generated).hasSize(1);
    List<String> warnings = getWarningStrings(results);
    assertThat(warnings).hasSize(1);
  ...
  public void testValidDocumentInBadJavaBean(Results results) throws IOException {
    assertThat(results.generated).hasSize(1);

    List<String> warnings = getWarningStrings(results);
    assertThat(warnings).hasSize(3);
    assertThat(warnings).contains("Class BadBean is not a proper JavaBean because id has no standard getter. Fix the issue to ensure stability.");
    assertThat(warnings).contains("Class BadBean is not a proper JavaBean because name has no standard getter. Fix the issue to ensure stability.");
    assertThat(warnings).contains("Class BadBean is not a proper JavaBean because age has no standard getter. Fix the issue to ensure stability.");
  }
